...
from src.transformers.training_args import TrainingArguments, OptimizerNames, ParallelMode
...
class Trainer:
    ...
    def train(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                # Added by ###### {
                self.train_idxs.extend(inputs["idx"])
                # Added by ###### }
            ...
            # Added by ###### { Here is after running all the steps in each epoch (Saving logits)
            ...
            if args.save_training_dynamics:
                save_tensor(torch.cat(self.train_logits), "training_dynamics", "train_logits")
                save_tensor(torch.stack(self.train_idxs), "training_dynamics", "train_idxs")
            ...
            if args.save_training_dynamics_after_epoch:
                extract_after_epoch_logits()
            # Added by ###### }
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.autocast_smart_context_manager():
            ...
            # Added by ###### {
            loss, outputs = self.compute_loss(model, inputs, return_outputs=True)
            self.train_logits.append(outputs.logits.clone().detach())
            # Added by ###### }
